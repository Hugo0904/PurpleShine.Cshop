...
using MegaMedia.Database.Redis.Expansion;
using MegaMedia.Logging.log4net;
...
namespace MegaMedia.Database.Redis
{
    public class RedisPoolManager : RedisNativeClient
    {
        ...
        public bool AddChannelEventListener(string channel, Func<bool> removeCondition, Action<string> eventTriger)
        {
            ...
            {
                LogR.Fatal("Default", $"AddChannelEventListener發生Exception, Message: {ex.Message}, Reason: {ex.StackTrace}");
            }
            return false;
        }
        ...
        public void CheckChannelListenersAlive()
        {
            ...
            {
                LogR.Debug("Default", "id: {0}, IsAlive: {1}, count: {2}, progress: {3}", i.Key.Client.Id, i.Key.Client.IsAlive(), i.Key.Subscription.SubscriptionCount, i.Key.Progress);
                if (!i.Key.Client.IsAlive() || i.Key.Subscription.SubscriptionCount == 0)
                {
                    LogR.Debug("Default", "Dispose clinet id {0}", i.Key.Client.Id);
                    ...
                    Thread thread = new Thread(() =>
                    {
                        ...
                        try
                        {
                            item.Subscription.SubscribeChannels(_chs, OnSubscribe, (channel, msg) =>
                            {
                                ...
                                item.Progress = 1;
                                LogR.Debug("Pubsub", "[{0} - {1}] {2} = {3}", Thread.CurrentThread.ManagedThreadId, item.Id, channel, msg);
                                ...
                                else
                                {
                                    ...
                                    if (item.Channels.Contains(channel) && EventListeners.TryGetValue(channel, out HashSet<EventListener> _items) && _items.Any())
                                    {
                                        ...
                                        Parallel.ForEach(_items.ToList(), (listener, loopState) =>   // 使用平行迭代
                                        {
                                            ...
                                            {
                                                LogR.Fatal("Default", "處理監聽到的channel事件發生Exception, channel: {0}, message: {1}, Message: {2}, Reason: {3}", channel, msg, ex.Message, ex.StackTrace);
                                            }
                                            item.Progress = 15;
                                        });
                                        item.Progress = 16;
                        ...
                        {
                            if (!close)
                                LogR.Fatal("Default", "監聽發生Exception, channel: {0}, Message: {1}, Reason: {2}", Json.Serialize(item.Channels), ex.Message, ex.StackTrace);
                        }
                        finally
                        {
                            ...
                            if (OnUnSubscribe != null)
                            {
                                ...
                                LogR.Fatal("Default", "監聽者已經退出, 歷史監聽頻道 = {0}", Json.Serialize(item.Channels));
                            }
                        }
                    })
                    {
